...
def get_multiclass_training_data(class_size=750):
    ...
    dictionary = project1.extract_dictionary(X_train)
    Y_train = X_train["label"].values.copy()
    X_train, vect = project1.generate_feature_matrix(X_train, dictionary)

    return (X_train, Y_train, vect)


def get_heldout_reviews(vect):
    """
    ...
    dataframe = load_data(fname)
    AHH = [x for x in dataframe['text']]
    X = vect.transform(AHH).toarray(order='C')
    return X
